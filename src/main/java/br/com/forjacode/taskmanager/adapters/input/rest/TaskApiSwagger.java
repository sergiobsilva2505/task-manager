...
public interface TaskApiSwagger {
    ...
    @ApiResponse(responseCode = "204", description = "Tarefa removida (ou já não existia/não era sua — resposta " +
            "idêntica)")
    @ApiResponse(responseCode = "400", description = "ID informado não é um UUID válido",
            content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
    @ApiResponse(responseCode = "401", description = "Token ausente, inválido ou expirado",
            content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
    ResponseEntity<Void> deleteTask(@Parameter(description = "ID da tarefa") UUID taskId,
            @Parameter(description = "ID do usuário atual") UUID currentUserId);
